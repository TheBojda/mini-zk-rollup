...
mkdir -p build
wget -nc https://hermez.s3-eu-west-1.amazonaws.com/powersOfTau28_hez_final_21.ptau -P ./build
[ ! -f ./build/verify-transfer-req-test.r1cs ] && circom test/circuits/verify-transfer-req-test.circom --wasm --r1cs -o ./build
[ ! -f ./build/verify-transfer-req-test.zkey ] && npx snarkjs groth16 setup build/verify-transfer-req-test.r1cs build/powersOfTau28_hez_final_21.ptau build/verify-transfer-req-test.zkey
...
[ ! -f ./build/rollup-tx-test.zkey ] && npx snarkjs groth16 setup build/rollup-tx-test.r1cs build/powersOfTau28_hez_final_21.ptau build/rollup-tx-test.zkey
...
[ ! -f ./build/rollup.zkey ] && npx snarkjs groth16 setup build/rollup.r1cs build/powersOfTau28_hez_final_21.ptau build/rollup.zkey
